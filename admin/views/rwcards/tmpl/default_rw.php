...
<?php endif; ?>
	...
	<div class="clr"> </div>
        <table class="table table-striped table-hover" id="rwcardsList">
			<thead>
				<tr>
					<!-- <th width="1%">
						<?php //echo JHtml::_('searchtools.sort', '<i class="icon-menu-2"></i>', 'ordering', $listDirn, $listOrder, null, 'asc', 'JGRID_HEADING_ORDERING');?>
					</th> -->
					               <th width="1%">
                   <?php echo JHtml::_('searchtools.sort', '', 'ordering', $listDirn, $listOrder, null, 'asc', 'JGRID_HEADING_ORDERING', 'icon-menu-2'); ?>
					...
					<th>
						<?php echo JHtml::_('searchtools.sort', 'COM_RWCARDS_RWCARDS_AUTOR', 'a.autor', $listDirn, $listOrder); ?>
					</th>
					<th>
...
$canEdit = $user->authorise('core.edit', 'com_rwcards.rwcards.' . $item->id);
...
?>
                ...
                ?>
						...
						<div class="pull-left">
                            ...
                            JHtml::_('dropdown.edit', $item->id, 'rwcard.');
                            JHtml::_('dropdown.divider');
                            if ($item->category_kategorien_name) :
                                JHtml::_('dropdown.unpublish', 'cb' . $i, 'rwcards.');
                            else :
                                JHtml::_('dropdown.publish', 'cb' . $i, 'rwcards.');
                            endif;

                            JHtml::_('dropdown.divider');

                            if ($archived) :
                                JHtml::_('dropdown.unarchive', 'cb' . $i, 'rwcards.');
                            else :
                                JHtml::_('dropdown.archive', 'cb' . $i, 'rwcards.');
                            endif;

                            if ($item->checked_out) :
                                JHtml::_('dropdown.checkin', 'cb' . $i, 'rwcards.');
                            endif;

                            if ($trashed) :
                                JHtml::_('dropdown.untrash', 'cb' . $i, 'rwcards.');
                            else :
                                JHtml::_('dropdown.trash', 'cb' . $i, 'rwcards.');
                            endif;
