	...
	<script type="text/javascript">
		...
		(function($) {
			window.addEvent('domready', function() {
				myFormValidator = new Form.Validator.Inline($('rwCardsForm'), {
					...
					onElementValidate: function(passed, element, validator, is_warn) {
						...
						if (element.get('name') == 'captchaCode' && !passed) {
							alert('<?php echo JText::_('COM_RWCARDS_FORM_CAPTURE_ERROR_MESSAGE'); ?>');
						}
					}
				});
				...
				Form.Validator.add('validate-dsgvo', {
					errorMsg: '<?php echo JText::_('COM_RWCARDS_FORM_ERROR_DSGVO'); ?>',
					test: function(element) {
						if (element.value.length == 0) return false;
						else return true;
					}
...
<?php
	...
	<h1><?php echo JText::_('COM_RWCARDS_PLEASE_FILL_OUT'); ?></h1>
	...
	<form name="rwCardsForm" id="rwCardsForm" method="post" action="<?php echo JURI::current(); ?>">
		...
		<div class="rwcardsfull">
			<div class="rwcardsLeftForm">
				...
				<div id="rwNameFrom">
					<div><label for="rwCardsFormNameFrom"><?php echo JText::_('COM_RWCARDS_NAME_FROM'); ?></label></div>
				...
				<div id="rwEmailFrom">
					<div><label for="rwCardsFormEmailFrom"><?php echo JText::_('COM_RWCARDS_EMAIL_FROM'); ?></label></div>
				...
				<div id="rwMessage">
					<div><label for="rwCardsFormMessage"><?php echo JText::_('COM_RWCARDS_MESSAGE'); ?></label></div>
			...
			<div class="rwcardsRightForm">
				...
				?>
						<div class="rwcardsReceiver">
							...
							<div><label for="rwCardsFormEmailTo[<?php echo $i; ?>]"><?php echo JText::_('COM_RWCARDS_EMAIL_TO'); ?></label></div>
				...
				<div id="moreReceivers"><?php echo JText::_('COM_RWCARDS_ADD_RECEIVER'); ?></div>
				<div id="lessReceivers" style="visibility:hidden;"><?php echo JText::_('COM_RWCARDS_REMOVE_RECEIVER'); ?></div>
...
-->
			...
			<div class="rwcards_dsgvo">
				<p><?php echo JText::_('COM_RWCARDS_FORM_DSGVO_PFLICHTFELDER'); ?></p>
				<p><?php echo JText::_('COM_RWCARDS_FORM_DSGVO_HINT'); ?></p>
				<div>
					<input id="rwCardsFormDsvgoCheckbox" name="rwCardsFormDsvgoCheckbox" onclick="setValue();" value="" <?php echo ($_SESSION['rwcardsSession']['rwCardsFormDsvgoCheckbox'] == 1) ? 'checked' : ''; ?> type="checkbox" class="validate-dsgvo rwcards_inputbox" />
					<span><?php echo JText::_('COM_RWCARDS_FORM_DSGVO_CHECKBOXHINT'); ?></span>
				</div>
			</div>
			<div class="rwcards_buttons_bottom">
				<input type="button" id="rwCardsBack" value="<?php echo JText::_('COM_RWCARDS_BACK'); ?>" class="rwcards_button" />
				<input type="submit" name="submit" id="rwCardsPreviewCard" value="<?php echo JText::_('COM_RWCARDS_PREVIEW_CARD'); ?>" class="rwcards_button" />
				<input type="submit" name="submit" id="rwCardsSendCard" value="<?php echo JText::_('COM_RWCARDS_SEND_CARD'); ?>" class="rwcards_button" />
			</div>
		</div>
...
<script type="text/javascript">
	...
	function setValue() {
		var checkbox = document.querySelector('#rwCardsFormDsvgoCheckbox');
		if (checkbox.checked) {
			checkbox.value = "1";
		} else {
			checkbox.value = "";
